...
boatsRouter.get('/:boatId',
  [param("boatId").notEmpty()], ReturnValidationErrors,
  async (req: Request, res: Response) => {
  ...
  const response = await db.insert(boat)
    ...
    .then(async rows => {
      ...
      if (ownerNewArray.length) {
        const newOwners = ownerNewArray.map((owner: any) => ({ ...owner, BoatId: newBoat.Id }))
      ...
      await db.insert(pastNamesNewArray.map((name: any) => ({ BoatId: newBoat.Id, ...name })))
        .into('boat.pastnames')
        .then(rows => {
          return rows;
        });
      ...
      if (histories.length) {
        const newHistories = histories.map((history: any) => ({ ...history, UID: newBoat.Id }))
...
boatsRouter.put('/:boatId', async (req: Request, res: Response) => {
  ...
  */
  ...
  await db.insert(ownerNewArray.map((owner: any) => ({ BoatId: boatId, ...owner })))
    .into('boat.boatowner')
    .then(rows => {
      return rows;
    });
  ...
  //Add the new past names (done)
  await db.insert(pastNamesNewArray.map((name: any) => ({ BoatId: boatId, ...name })))
    .into('boat.pastnames')
    .then(rows => {
      return rows;
    });
