...
ownerRouter.get('/:ownerId',
  [param("ownerId").notEmpty()], ReturnValidationErrors,
  async (req: Request, res: Response) => {
    /*  const permissions = req.decodedToken['yg-claims'].permissions;
     if (!permissions.includes('view')) res.sendStatus(403);
...
ownerRouter.put('/:ownerId',
  ...
  async (req: Request, res: Response) => {
    ...
    newArray = newOwnerAlias.map((alias: any) => { return { OwnerId: ownerId, ...alias } });
  ...
  const response = await db.insert(owner)
    ...
    .then(async rows => {
      ...
      if (ownerAlias.length) {
        const newOwnerAlias = ownerAlias.map((alias: any) => ({ ...alias, OwnerId: newOwner.Id }))
