...
aircrashRouter.put('/:aircrashId',
  ...
  async (req: Request, res: Response) => {
    ...
    await db.insert(newInfoSources.map((source: any) => ({ YACSINumber: aircrashId, ...source })))
    ...
    const response = await db.insert(aircrash)
      ...
      .then(async rows => {
        ...
        if (newInfoSources.length) {
          const finalInfoSources = newInfoSources.map((source: any) => ({ YACSINumber: newAirCrash.YACSINumber, ...source }))
...
aircrashRouter.get('/available_yacsi/:YACSINumber',
  [param("YACSINumber").notEmpty()], ReturnValidationErrors,
  async (req: Request, res: Response) => {
    /* const permissions = req.decodedToken['yg-claims'].permissions;
    if (!permissions.includes('view')) res.sendStatus(403);
