...
catalogsRouter.put(
	...
	[param('communityId').notEmpty()],
	ReturnValidationErrors,
	async (req: Request, res: Response) => {
		const { communityId } = req.params;
		const { community = {} } = req.body;

		await db('Community').update(community).where('community.id', communityId);

		res.status(200).send({ message: 'success' });
	}
);

catalogsRouter.get('/photo-owner', async (req: Request, res: Response) => {
	const { textToMatch = '', sortBy = 'Id', sort = 'asc' } = req.query;
	const page = parseInt(req.query.page as string);
	const limit = parseInt(req.query.limit as string);
	const offset = page * limit || 0;
...
catalogsRouter.get('/photo-project', async (req: Request, res: Response) => {
	const { textToMatch = '', sortBy = 'Id', sort = 'asc' } = req.query;
	const page = parseInt(req.query.page as string);
	const limit = parseInt(req.query.limit as string);
	const offset = page * limit || 0;
...
catalogsRouter.get('/photo-subject', async (req: Request, res: Response) => {
	const { textToMatch = '', sortBy = 'Id', sort = 'asc' } = req.query;
	const page = parseInt(req.query.page as string);
	const limit = parseInt(req.query.limit as string);
	const offset = page * limit || 0;
