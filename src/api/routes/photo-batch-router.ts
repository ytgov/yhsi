...
photoBatchRouter.get("/",
    ...
    async (req: Request, res: Response) => {
        ...
        let list = await photoBatchService.getAll(skip, take)
            .then(data => data)
            .catch((err) => { console.error("Database Error", err); return undefined; });

        let item_count = await photoBatchService.getPhotoBatchCount()
            .then(data => data)
...
photoBatchRouter.post("/search", [body("page").isInt().default(1)],
    async (req: Request, res: Response) => {
...
photoBatchRouter.get("/:id",
    ...
    async (req: Request, res: Response) => {
        ...
        await photoBatchService.getById(req.params.id)
            .then(item => {
                if (item)
...
photoBatchRouter.post("/",
    ...
    async (req: Request, res: Response) => { 
        ...
        let result = await photoBatchService.addBatch(req.body as PhotoBatch).then(item => item)
            .catch(err => {
                console.log(err);
...
photoBatchRouter.get("/:id/photos",
    [
        check("id").notEmpty()
    ...
    async (req: Request, res: Response) => {
        ...
        let list = await photoBatchService.getPhotos(req.params.id)
            .then(data => data)
            .catch((err) => { console.error("Database Error", err); return undefined; });
...
photoBatchRouter.put("/:id",
    ...
    async (req: Request, res: Response) => {
        ...
        let result = await photoBatchService.updateBatch(req.params.id, req.body as PhotoBatch).then(item => item)
            .catch(err => {
                console.log(err);
                return res.json({ errors: [err.originalError.info.message] });
            });
...
photoBatchRouter.delete("/photo/:id",
    [check("id").isInt().notEmpty()],
    async (req: Request, res: Response) => {
        const errors = validationResult(req);
...
photoBatchRouter.post("/photo", multer().single("file"),
    ...
    async (req: Request, res: Response) => {
        ...
        let result = await photoBatchService.addPhoto(req.body as PhotoBatchPhoto).then(item => item)
            .catch(err => {
                console.log(err);
                return res.json({ errors: [err.originalError.info.message] });
            });
...
photoBatchRouter.put("/:id/process-batch",
    [check("id").isInt().notEmpty()],
    async (req: Request, res: Response) => { 
        const errors = validationResult(req); 
        ...
        let result = await photoBatchService.processBatch(req.params.id).then(item => item)
            .catch(err => {
                console.log(err);
                return res.json({ errors: [err.originalError.info.message] });
            });
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            if (photo && photo.file) {
                ...
                let res = await photoService.updateThumbFile(result[i].rowid,thumbnail);
            }
        };
