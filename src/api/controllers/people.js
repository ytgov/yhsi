...
router.get('/', authenticateToken, async (req, res) => {
  ...
  const { page = 0, limit = 10, textToMatch = '', sortBy = 'PersonId', sort = 'asc' } = req.query;
  const offset = (page*limit) || 0;
  let counter = 0;
  let people = [];

  if (textToMatch) {
    counter = await db.from('Person.Person')
    ...
    people = await db.from('Person.Person')
    .where('Surname', 'like', `%${textToMatch}%`)
    .orWhere('GivenName', 'like', `%${textToMatch}%`)
    .orWhere('BirthYear', 'like', `%${textToMatch}%`)
    .orWhere('BirthAccuracy', 'like', `%${textToMatch}%`)
...
router.get('/:personId', authenticateToken, async (req, res) => {
  const db = req.app.get('db');
  ...
  const person =  await db.from('Person.Person')
...
//Modify Person
router.put('/:personId', authenticateToken, async (req, res) => {
  const db = req.app.get('db');
  const permissions = req.decodedToken['yg-claims'].permissions;
...
router.post('/', authenticateToken, async (req, res) => {
    ...
    const newPerson = await db.insert(person)
        .into('Person.Person')
        .returning('*');
...
//Add history
router.post('/:personId/history', authenticateToken, async (req, res) => {
    const db = req.app.get('db');
    const permissions = req.decodedToken['yg-claims'].permissions;
    if (!permissions.includes('edit')) res.sendStatus(403);
    ...
    const newHistory = await db.insert(history)
        .into('Person.History')
        .returning('*');
