...
import { Photo, PhotoBatch, PhotoBatchPhoto, PHOTO_FIELDS, PHOTO_BATCH_FIELDS, SavedFilter } from "../data";
...
export class PhotoBatchService {
    ...
    async getAll(skip: number, take: number): Promise<Array<any>> {
        return this.knex("photobatch").select(PHOTO_BATCH_FIELDS)
        ...
        .groupBy(PHOTO_BATCH_FIELDS).groupBy(this.knex.raw("photobatch.id"))
        .orderBy("photobatch.name").offset(skip).limit(take);
    }

    async getPhotoBatchCount(): Promise<number> {
        return new Promise(async (resolve, reject) => {
            let results = await this.knex<number>("photobatch").count("*", { as: 'count' })
    ...
    async doSearch(query: Array<QueryStatement>, sort: Array<SortStatement>, page: number, page_size: number, skip: number, take: number): Promise<any> {
        return new Promise(async (resolve, reject) => {
            ...
            let selectStmt = this.knex("photobatch").select(PHOTO_BATCH_FIELDS)
            .select(this.knex.raw("photobatch.id as batchId"))
            .select(this.knex.raw("photobatch.userId as userName"))
            .leftOuterJoin("photobatchphoto", "photobatch.id", "photobatchphoto.photobatchid")
            .count("photobatchphoto.id", { as: 'photoCount' })
            .groupBy(PHOTO_BATCH_FIELDS).groupBy(this.knex.raw("photobatch.id")).groupBy(this.knex.raw("photobatch.userId"))
    ...
    async getById(id: string): Promise<PhotoBatch | undefined> {
        return this.knex("photobatch").select<PhotoBatch>(PHOTO_BATCH_FIELDS).where({ id: id }).first()
    ...
    async getPhotos(id: string): Promise<PhotoBatchPhoto | undefined> {
        return this.knex("photobatchphoto").select<PhotoBatchPhoto>(['id','photoBatchId','photoFile','photoFileName','photoContentType'])
        .where({ photoBatchId: id })
        .catch(err => { console.log("BOMBED", err); return undefined; })
    }

    async processBatch(id: string): Promise <any> {    
        return this.knex.raw(`insert into Photo (
    ...
    async updateBatch(id: string, item: PhotoBatch): Promise<PhotoBatch | undefined> {
        return this.knex("PhotoBatch").where({ id: id }).update(item).returning<PhotoBatch>(PHOTO_BATCH_FIELDS);
    }
    ...
    async addPhoto(item: PhotoBatchPhoto): Promise<PhotoBatchPhoto | undefined> {
        return this.knex("PhotoBatchPhoto").insert(item).returning<PhotoBatchPhoto>(['id','photoBatchId','photoFile','photoFileName','photoContentType']);
    }
