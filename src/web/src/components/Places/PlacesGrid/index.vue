...
export default {
	...
	methods: {
		...
		handleClick(value) {
			//Redirects the user to the place form component
			this.$router.push({
				name: 'placeView',
			...
			axios
				...
				.then((resp) => {
					...
					this.places.forEach((val) =>
						this.placesCsv.push(Object.assign({}, val))
					);
					this.placesCsv.map((x) => {
						x.placeTypes = x.placeTypes.map((x) => (x = x.placeType));
						x.alternateNames = x.alternateNames.map(
							(x) => (x = x.alternateName)
						);
	...
	computed: {
		...
		filteredData() {
			// returns a filtered places array depending on the selected filters
