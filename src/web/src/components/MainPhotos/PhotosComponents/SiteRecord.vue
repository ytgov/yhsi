                    ...
                    <v-card color="#fff2d5"
                      ...
                      <v-card-text>
                        <v-btn v-if="mode == 'edit'"
                          ...
                          class="float-right"
                          style="margin-top: -6px!important;margin-bottom: 0px!important;"
                          @click="deleteAssociation('interpretive-sites',item.siteId)"
                          >
                          <v-icon>mdi-close</v-icon>
                        </v-btn
                        >
...
export default {
  ...
  created(){
    if(this.fields.rowId) {
      ...
      axios.get(`${PHOTO_URL}/${this.fields.rowId}/interpretive-sites`).then((resp) => {
  ...
  methods:{
    ...
    deleteAssociation(itemType, itemId) {
      axios.delete(`${PHOTO_URL}/${this.fields.rowId}/${itemType}/${itemId}`).then(() => {
        switch (itemType) {
          case 'place' : {
